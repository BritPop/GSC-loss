...
parser.add_argument('--eval_frames',    type=int,   default=600,      help='Input length to the network for testing 0 uses the whole files')
parser.add_argument('--num_eval',       type=int,   default=10,      help='Input length to the network for testing 0 uses the whole files')
parser.add_argument('--num_spk',        type=int,   default=200,    help='Number of speakers per batch, i.e., batch size = num_spk * num_utt')
...
parser.add_argument('--num_thread',     type=int,   default=8,      help='Number of loader threads')
...
parser.add_argument('--trainfunc',      type=str,   default="AAM_sc_intertopk", help='Loss function')
...
parser.add_argument('--lr',             type=float, default=0.0001,  help='StepLR sched: Learning rate')
...
parser.add_argument('--lr_wstep',       type=int,   default=1,      help='Cosine sched: Linear warmup step size')
...
parser.add_argument('--w_cls',          type=float, default=31.0,    help='Weight for softmax-based loss')
parser.add_argument('--w_mtr',          type=float, default=1.0,    help='Weight for metric-based loss')
parser.add_argument('--hard_n',          type=int, default=10000,    help='Number of negatives for hard negative mining')
parser.add_argument('--hard_d',          type=float, default=0,    help='margin between negatives and minimum positive for hard negative mining')
parser.add_argument('--hard_p',          type=int, default=0,    help='Number of negatives for hard negative mining')
...
parser.add_argument('--save_path',      type=str,   default="./save/wavlm_ecapa_aam_sc_intertopk", help='Path for model and logs')

## Training and test data
parser.add_argument('--train_list',     type=str,   default="./list/train_vox1-vox2_mhhan.txt", help='Train list')
parser.add_argument('--test_list',      type=str,   default="./list/veri_test2.txt", help='Evaluation list')
parser.add_argument('--train_path',     type=str,   default="/home/mhhan/DB/VoxCeleb/VoxCeleb-all/wav/",     help='Absolute path to the train set')
parser.add_argument('--test_path',      type=str,   default="/home/mhhan/DB/VoxCeleb/VoxCeleb_test_all/", help='Absolute path to the test set')
parser.add_argument('--musan_path',     type=str,   default="/home/mhhan/DB/MUSAN/musan_split", help='Absolute path to the test set')
parser.add_argument('--rir_path',       type=str,   default="/home/mhhan/DB/RIRS_NOISES/simulated_rirs", help='Absolute path to the test set')
...
parser.add_argument('--model',          type=str,   default="WavLM_ECAPA_TDNN", help='Name of model definition')
...
parser.add_argument('--jt',      type=bool,  default=False,   help='joint training option for WavLM')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for it in range(it,args.max_epoch+1):
        ...
        loss, traineer, traineer2, N_N, lr = trainer.train_network(train_loader, it, verbose=(args.gpu == 0))
        if args.gpu == 0:
            print('')
        ...
        if it % args.test_interval == 0:
            ...
            if args.gpu == 0:
                ...
                EERs1 += [eer1]
                DCFs1 += [dcf1]
                print('\n',time.strftime("%Y-%m-%d %H:%M:%S"), "Epoch {:d}, ACC {:2.2f}, ACC2 {:2.2f}, N_N {:.2f} TLOSS {:f}, LR {:f}, EER {:2.4f}, MinDCF {:2.5f}, bestEER {:2.4f}, bestMinDCF {:2.5f}".format(it, \
                                                                                                                                                                                           traineer, traineer2, N_N,\
                                                                                                                                                                                           loss, \
                                                                                                                                                                                           lr, \
                                                                                                                                                                                           eer1, \
                                                                                                                                                                                           dcf1, \
                                                                                                                                                                                           min(EERs1), \
                                                                                                                                                                                           min(DCFs1)))
                scorefile.write("Epoch {:d}, ACC {:2.2f}, ACC2 {:2.2f}, N_N {:.2f} TLOSS {:f}, LR {:f}, EER {:2.4f}, MinDCF {:2.5f}, bestEER {:2.4f}, bestMinDCF {:2.5f}\n".format(it, traineer, traineer2, N_N, loss, lr, eer1, dcf1, min(EERs1), min(DCFs1)))
